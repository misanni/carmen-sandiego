...
--been traveling through Southern Europe. She's most likely traveling someplace where she 
--won't be noticed, so find the least populated country in Southern Europe, and we'll start 
...
--classes in this country's officially recognized language. Check our databases and find 
--out what language is spoken in this country, so we can call in a translator to work with you.
SELECT "language" FROM countrylanguage
WHERE countrycode = (SELECT code FROM country WHERE name = 'Holy See (Vatican City State)')
AND isofficial = true;
...
--hance to catch her this time. There are only two cities she could be flying 
--to in the country. One is named the same as the country – that would be too 
--obvious. We're following our gut on this one; find out what other city in that
...
--names, but in totally different parts of the globe! She's headed to South America 
--as we speak; go find a city whose name is like the one we were headed to, but 
--doesn't end the same. Find out the city, and do another search for what country 
--it's in. Hurry!SELECT * FROM city
WHERE name LIKE '%Serra%'
AND name NOT LIKE '%Serravalle'
AND countrycode IN (
 SELECT code
 FROM country
...
--behind you!
SELECT * FROM city WHERE id='265';



-- Clue #7: She knows we're on to her – her taxi dropped her off at the international
-- airport, and she beat us to the boarding gates. We have one chance to catch her,
-- we just have to know where she's heading and beat her to the landing dock.

-- Lucky for us, she's getting cocky. She left us a note, and I'm sure she thinks
-- she's very clever, but if we can crack it, we can finally put her where she belongs – behind bars.
